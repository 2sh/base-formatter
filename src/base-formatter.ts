...
/**
 * - `'auto'`: Sign display for negative numbers only, including
 *   negative zero.
 * - `'always'`: Always display sign.
 * - `'exceptZero'`: Sign display for positive and negative numbers,
 *   but not zero.
 * - `'negative'`: Sign display for negative numbers only, excluding
 *   negative zero.
 ...
 * - `'halfCeil'`: Values above or equal to the half-increment round
 *   like `ceil`, otherwise like `floor`.
 * - `'halfFloor'`: Values above the half-increment round like `ceil`,
 *   otherwise like `floor`.
 * - `'halfExpand'`: Values above or equal to the half-increment round
 *   like `expand`, otherwise like `trunc`.
 * - `'halfTrunc'`: Values above the half-increment round like `expand`,
 *   otherwise like `trunc`.
 * - `'halfEven'`: Like `halfExpand`, except that on the half-increment
 *   values round towards the nearest even digit.
 * - `'halfOdd'`: Like `halfExpand`, except that on the half-increment
 *   values round towards the nearest odd digit.
 */
 ...
 * - `'min2'`: Display grouping separators when there are at least 2
 *   digits in a group.
...
/**
 * The options passed to {@link BaseConverter | `BaseConverter`} and
 * {@link BaseConverter.encode | `BaseConverter.encode`}.
 */
export interface BaseConverterOptions
{
   ...
   * By default, the maximum fraction length is determined by the
   * precision.
   * @defaultValue `null`
   */
...
{
  /**
   * The radix character, or "decimal" point/mark/separator, such as the
   * point in `0.5`).
   * @defaultValue `'.'`
   */
  ...
  /**
   * The grouping length, the distance between grouping separators, e.g.
   * with a length of 2: `1,00,00,00`.
   * @defaultValue `3`
   */
  groupingLength?: number
  /**
   * The digit separator, if specified, will be places between every
   * digit without a grouping separator.
   * @defaultValue `''`
   */
   ...
   * A value with a smaller number of integer digits than this number
   * will be left-padded with zeros or the specified integer pad
   * character.
   * @defaultValue `0`
   */
 ...
 * {@link BaseFormatter.encode | `BaseFormatter.encode`} and
 * {@link BaseFormatter.decode | `BaseFormatter.decode`}.
 * It includes the options for the {@link BaseConverter | `BaseConverter`}.
 */
export interface BaseFormatterOptions
  extends BaseConverterOptions, BaseFormatterOnlyOptions { }

/**
 * The numeral output of the {@link BaseConverter.encode | `BaseConverter.encode`}
 * method.
 */
 ...
 */
function convertFractionToBase(
  value: Decimal, base: number, maximumLength: Decimal): Decimal[]
{
  const baseVal: Decimal[] = []
...
/**
 * The class from which to create a `BaseConverter` instance for
 * encoding to the {@link NumeralOutput | `NumeralOutput`}.
 */
export class BaseConverter
{
  ...
  private readonly roundingModes: {
    [mode in RoundingMode]: (
      value: Decimal,
      isNegative: boolean,
      index: number,
      values: (Decimal | null)[]) => boolean
  }
  /**
   * The natural logarithm of the base used by the {@link calculateExponent | `calculateExponent`}
   * method.
   */
  private readonly lnBase: Decimal
  /**
   * The natural logarithm of base*10^3 used by the {@link calculateExponent | `calculateExponent`}
   * method.
   */
   ...
   * @param options - Optional parameters, for adjusting conversion
   * rules.
   */
  constructor (base: number, options: BaseConverterOptions = {})
  {
    ...
    this.roundingModes = {
      ...
      halfCeil: (value, isNegative) => !isNegative
        ? value.greaterThanOrEqualTo(halfBase)
        : value.greaterThan(halfBase),
      halfFloor: (value, isNegative) => isNegative
        ? value.greaterThanOrEqualTo(halfBase)
        : value.greaterThan(halfBase),
  ...
  private calculateExponent(
    value: Decimal | number, isEngineering = false): number
  {
  ...
  /**
   * Encodes the specified number according to the instance base and
   * properties.
   * @param numberValue - The number to encode, as a number, string or
   * Decimal type.
   * @param options - The options to use as conversion rules.
   * @returns The encoded number as a {@link NumeralOutput | `NumeralOutput`}
   * object.
   * @group Instance Methods
   */
  public encode(
    numberValue: number | string | Decimal,
    options?: BaseConverterOptions): NumeralOutput
  {
    ...
    const exponent = this.calculateExponent(
      absVal, props.notation === 'engineering')

    const makeExponential = props.notation !== 'standard' && exponent != 0
    const expValue = makeExponential
      ? (absVal.dividedBy(Decimal.pow(this.base, exponent)))
      : absVal
    ...
    const maxFractLengthByPrecision = (precisionExponent > 0
      ...
      : decPrecision) || zero
    const maxFractionalLength = (
      typeof props.maximumFractionLength == "number"
    ...
    const baseFractVal = convertFractionToBase(
      fractVal, this.base, maxFractLengthByPrecision)
    ...
    for (let i = baseVal.length - 1; i >= 0; i--)
    {
      ...
      {
        if (this.roundingModes[props.roundingMode](
          value, isNegative, i, baseVal))
          isRemainder = true
      }
  ...
  public decode(encodedValue: NumeralOutput): number
  {
    ...
    return [...encodedValue.integer, ...encodedValue.fraction]
      .reduce((a, c, i) => a + c * Math.pow(this.base, largestExponent - i), 0)
      * Math.pow(
        this.base, encodedValue.exponent) * (encodedValue.isNegative ? -1 : 1)
  }
}
...
/**
 * The class from which to create a `BaseFormatter` instance for
 * encoding to a string format from specified digits.
 */
export class BaseFormatter
{
  ...
  /**
   * The zero character of the specified digits. Becomes a `' '` space
   * character if no digits are specified.
   */
  private readonly baseZero: string

  /**
   * The constructor of the `BaseFormatter` class.
   * @param digits - A string of symbols for representing the digits,
   * the length of which determining the base number.
   * @param options - Optional parameters, for adjusting conversion
   * rules and the encoding output string formatting.
   */
  constructor (
    digits: string | string[], options: BaseFormatterOptions = {})
  {
  ...
  /**
   * This method will take the base number, slice a string of digits
   * 0-9A-Za-z to a length equal to the base number and return an
   * instance of the {@link BaseFormatter | `BaseFormatter`} class with
   * the sliced string as the digits.
   * @param base - The base number to use, a maximum of `62`.
   * @param options - The options to use.
   * @returns An instance of the {@link BaseFormatter | `BaseFormatter`}
   * class.
   * @throws {@link MaximumBaseExceeded | `MaximumBaseExceeded`} when
  ...
  {
    if (base > allDigits.length)
      throw new MaximumBaseExceeded(
        "Can't be higher than "
        + allDigits.length.toString()
        + " digits")
    return new BaseFormatter(
      [...allDigits].slice(0, base).join(''), options)
  }

  /**
   * This method returns an instance of the {@link BaseFormatter | `BaseFormatter`}
   ...
   * @group Static Methods
   */
  public static binary(options?: BaseFormatterOptions)
  { return BaseFormatter.byBase(2, options) }
   ...
   * @group Static Methods
   */
  public static octal(options?: BaseFormatterOptions)
  { return BaseFormatter.byBase(8, options) }
   ...
   * @group Static Methods
   */
  public static decimal(options?: BaseFormatterOptions)
  { return BaseFormatter.byBase(10, options) }
   ...
   * @group Static Methods
   */
  public static hexadecimal(options?: BaseFormatterOptions)
  { return BaseFormatter.byBase(16, options) }
  ...
  {
    return new BaseFormatter(
      numbers + '↊↋', { radixCharacter: ';', ...options })
  }
   ...
   * the digits ↊ and ↋ used by the {@link BaseFormatter.dozenal | `BaseFormatter.dozenal`}
  ...
  {
    return new BaseFormatter(
      numbers + 'TE', { radixCharacter: ';', ...options })
  }
  ...
  {
    return new BaseFormatter(
      numbers + 'XE', { radixCharacter: ';', ...options })
  }
   ...
   * @group Static Methods
   */
  public static duodecimal(options?: BaseFormatterOptions)
  { return BaseFormatter.byBase(12, options) }
   ...
   * @group Static Methods
   */
  public static vigesimal(options?: BaseFormatterOptions)
  { return new BaseFormatter(numbers + "ABCDEFGHJK", options) }
   ...
   * @group Static Methods
   */
  public static base62(options?: BaseFormatterOptions)
  { return BaseFormatter.byBase(62, options) }
  /**
   * This method returns an instance of the {@link BaseFormatter | `BaseFormatter`}
  ...
  private encodeInteger(
    value: (number | string)[], opts: BaseFormatterProperties): string
  {
    return value
   ...
   * @throws {@link DigitNotFound | `DigitNotFound`} when no digit could
   * be found for the specified number.
   * @group Instance Methods
   */
  private encodeDigit(value: number): string
  {
    if (!this.digits[value])
      throw new DigitNotFound('No digit found for number specified')
   ...
   * @throws {@link DigitNotFound | `DigitNotFound`} when the string is
  ...
  private decodeDigit(value: string): number
  {
    ...
    if (!(digitIndex >= 0))
      throw new DigitNotFound('String value not found in digits')
    return digitIndex
  }

  /**
   * Encodes the specified number according to the instance base and
   * specified digits and properties.
   * @param numberValue - The number to encode, as a number, string or
   * Decimal type.
  ...
  public encode(
    numberValue: number | string | Decimal,
    options?: BaseFormatterOptions): string
  {
    ...
    props.minimumFractionLength = converterProps.maximumFractionLength
      ? Math.min(props.minimumFractionLength,
        converterProps.maximumFractionLength)
      : props.minimumFractionLength

    const decVal = new Decimal(numberValue)

    const { isNegative, integer, fraction, exponent } =
      this.baseConverter.encode(numberValue, options)

    const encodedIntVal = this.encodeInteger(
      (props.minimumIntegerLength && props.minimumIntegerLength > integer.length
        ? [...createPadArray(
          (new Decimal(props.minimumIntegerLength))
            .minus(integer.length), props.integerPadCharacter),
        ...integer]
        : integer), props)

    const encodedFractVal =
      (props.minimumFractionLength > fraction.length
        ? [...fraction, ...createPadArray(
          new Decimal(props.minimumFractionLength - fraction.length),
          props.fractionPadCharacter)]
        : fraction)
        .map(n => typeof n === 'string' ? n : this.encodeDigit(n))
        .join('')
    ...
    const encodedExponent =
      converterProps.notation !== 'standard' && exponent != 0
        ? (props.scientificNotationCharacter + (this.encode(exponent, {
          ...props,
          notation: 'standard',
          minimumFractionLength: 0,
          maximumFractionLength: 0,
          minimumIntegerLength: 0
        })))
        : ''
    ...
    return outputSignSymbol
      + encodedIntVal
      + (encodedFractVal || props.radixDisplay === 'always'
        ? (props.radixCharacter + encodedFractVal)
        : '')
      + encodedExponent
  }
  ...
  /**
   * Decode an en encoded number according to the instance base and
   * specified digits and properties.
   * @param encodedValue - An encoded number as a string in the instance
   * base.
   * @param options - The options to use if, for example, alternative
   * characters were specified during encoding.
   * @returns The decoded number.
   * @throws {@link DigitNotFound | `DigitNotFound`} when the specified
   * string contains unknown characters.
   * @group Instance Methods
   */
  public decode(
    encodedValue: string,
    options?: BaseFormatterOptions): number
  {
    const opts: BaseFormatterProperties =
    ...
    const notationIndex =
      trimmedValue.indexOf(opts.scientificNotationCharacter)

    const exponent = notationIndex >= 0
      ? this.decode(trimmedValue.slice(notationIndex + 1))
      : 0
    const intFractValue = notationIndex >= 0
      ? trimmedValue.slice(0, notationIndex)
      : trimmedValue
    ...
    const integerArray = radixCharIndex >= 0
      ? valueArray.slice(0, radixCharIndex)
      : valueArray
    const fractionArray = radixCharIndex >= 0
      ? valueArray.slice(radixCharIndex + 1)
      : []
  ...
  /**
   * Check if the specified value is a number according to the instance
   * base and specified digits and properties.
